...
export class SqlGenericService {
    ...
    excecuteQueryStringReference(query: string, reference: string, whitReturn: number = 1, pushData:any = null, token:any = null) {
        let queryEncript: any = this.localStorageEncryptService.encryptBack(query);
        //console.log(query);
        ...
        if (sqlplit[0].toUpperCase() == "insert".toUpperCase()) {
            ...
            whitReturn = 3;
        } else if (sqlplit[0].toUpperCase() == "delete".toUpperCase()) {
            whitReturn = 4;
        }
        ...
        return this.genericService.sendPostRequest(`${environment.genericQuerie}/${reference}`, request);
    }
}